// Plate Analyzer — Profile screen

function ScreenProfile({ navigate, proMode = false }) {
  return (
    <div style={{ padding: '8px 16px 24px' }}>
      ...
      {/* identity card */}
      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--line)', borderRadius: 14, padding: 14, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{
        ...
        <button style={{ padding: '6px 10px', background: 'var(--bg-canvas)', border: '1px solid var(--line)', borderRadius: 6, color: 'var(--text-2)', fontFamily: 'var(--mono)', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em', cursor: 'pointer' }}>Éditer</button>
      </div>

      {proMode ? <PatientList navigate={navigate} /> : <PersonalProfile />}
      ...
      <div style={{ marginTop: 12, padding: '14px 12px', textAlign: 'center', fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--text-4)', lineHeight: 1.6 }}>
        ...
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--line)', borderRadius: 12, padding: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, paddingTop: 12, borderTop: '1px solid var(--line)' }}>
            {[
              ...
              <div key={i} style={{ padding: '8px 6px', background: 'var(--bg-canvas)', border: '1px solid var(--line)', borderRadius: 7, textAlign: 'center' }}>
          ...
          <div style={{ paddingTop: 12, marginTop: 12, borderTop: '1px solid var(--line)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
              <ProfileTag c="var(--alert)" l="Cholestérolémie" s="AGS ≤ 7%" />
              <ProfileTag c="var(--accent-2)" l="Méditerranéen" />
              <ProfileTag c="var(--ok)" l="Sport endurance" s="3×/sem" />
            </div>
          </div>
        </div>
      </PASection>
      ...
      <PASection title="Démographie" code="Mifflin-St Jeor">
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--line)', borderRadius: 12, padding: 14 }}>
          ...
          <div style={{ paddingTop: 12, borderTop: '1px solid var(--line)', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            <DemoStat l="IMC" v="22.1" status="ok" />
            <DemoStat l="MB" v="1 386" u="kcal" />
            <DemoStat l="Activité" v="×1.55" />
          </div>
        </div>
      </PASection>
    </>
      ...
      <PASection title="Patients suivis" action="+ Nouveau" code="4 / 4 actifs">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {patients.map((p, i) => {
            ...
            return (
              <div key={i} onClick={() => navigate && navigate('patient-detail')} style={{
                background: 'var(--bg-surface)', border: '1px solid var(--line)',
                borderRadius: 10, padding: 12, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer',
              }}>
...
function ProfileTag({ c, l, s }) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 8px', background: 'var(--bg-canvas)', border: '1px solid var(--line)', borderRadius: 5 }}>
...
function SettingsGroup({ rows }) {
  return (
    <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--line)', borderRadius: 12, overflow: 'hidden' }}>
      {rows.map((r, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', borderBottom: i < rows.length-1 ? '1px solid var(--line)' : 'none', color: r.danger ? 'var(--alert)' : 'var(--text-1)', cursor: 'pointer' }}>
          <div style={{ width: 22, height: 22, borderRadius: 5, background: r.danger ? 'var(--alert-soft)' : 'var(--bg-raised)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: r.danger ? 'var(--alert)' : 'var(--text-2)', flexShrink: 0 }}>
