// Plate Analyzer — Today · Capture · Loading

function ScreenToday({ navigate, dense = false }) {
  return (
    <div style={{ padding: '8px 16px 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0 18px' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--bg-surface)', border: '1px solid var(--line)', color: 'var(--text-4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--line)', borderRadius: 16, padding: 16, marginBottom: 12, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -20, right: -20, width: 140, height: 140, background: 'radial-gradient(circle, rgba(196,181,253,0.10), transparent 70%)', pointerEvents: 'none' }} />
      ...
      <PASection title="Indicateurs · vs OMS / PNNS" action={dense ? 'Réduire' : 'Détail'} code="J−0">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <TodayIndicator label="Sel" code="NaCl" v="4.2" u="g" target="5.0" status="warn" pct={84} sub="↑ 84% · 0.8g restants" />
          <TodayIndicator label="AG saturés" code="AGS" v="14.7" u="g" target="16.7" status="ok" pct={88} sub="cible adaptée" />
          <TodayIndicator label="Sucres libres" code="GS" v="42" u="g" target="50" status="ok" pct={84} sub="—" />
          <TodayIndicator label="Fibres" code="FA" v="18" u="g" target="30" status="warn" pct={60} sub="↓ 12g manquants" />
        </div>
        {dense && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
            <TodayIndicator label="Cholestérol" code="C" v="184" u="mg" target="300" status="ok" pct={61} sub="—" />
            <TodayIndicator label="Sodium" code="Na" v="1 680" u="mg" target="2 000" status="ok" pct={84} sub="—" />
          </div>
        )}
      </PASection>
      ...
      <PASection title="Repas du jour" action="+ Ajouter" code="3 · 1 487 kcal">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <MealRow time="08:14" name="Tartines avocat + œuf" kcal={412} nova={2} thumb="ok" navigate={navigate} />
          <MealRow time="12:48" name="Salade quinoa-feta" kcal={628} nova={2} thumb="cyan" navigate={navigate} />
          <MealRow time="16:22" name="Pomme + amandes" kcal={228} nova={1} thumb="warm" navigate={navigate} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 10, border: '1px dashed var(--line-strong)', borderRadius: 10, color: 'var(--text-3)' }}>
            <div style={{ width: 36, height: 36, borderRadius: 8, border: '1px dashed var(--line-strong)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</div>
            <div style={{ flex: 1, fontSize: 12 }}>Dîner</div>
      ...
      <PASection title="Alertes · cumul du jour">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <TodayAlert status="warn" title="Sel 4.2 / 5 g" sub="Limite restante : 0.8 g. Évitez plats préparés ce soir." />
          <TodayAlert status="alert" title="EPA+DHA insuffisants" sub="Aucun poisson gras depuis 6 jours. Cible 0.50 g/j." />
          <TodayAlert status="muted" title="Fibres en retard" sub="+12 g requis. Suggestion : légumineuses au dîner." />
        </div>
      </PASection>
      ...
      <button onClick={() => navigate('capture')} style={{
        ...
        background: 'var(--accent)', color: '#0a0d12', border: 'none', borderRadius: 12,
...
function TodayIndicator({ label, code, v, u, target, status, pct, sub }) {
...
function MicroStat({ l, v, u, t, ok, warn }) {
  ...
  return (
    <div>
      ...
      {t !== '—' && <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--text-4)', marginTop: 2 }}>cible {t}</div>}
    </div>
  );
}

function MealRow({ time, name, kcal, nova, thumb, navigate }) {
  const thumbColors = { ok: 'var(--ok)', cyan: 'var(--accent-2)', warm: 'var(--accent)' };
  const tc = thumbColors[thumb] || 'var(--accent)';
  return (
    <div onClick={() => navigate && navigate('result')} style={{
      display: 'flex', alignItems: 'center', gap: 10, padding: 8,
      background: 'var(--bg-surface)', border: '1px solid var(--line)', borderRadius: 10, cursor: 'pointer',
    }}>
      <div style={{ width: 38, height: 38, borderRadius: 8, background: 'var(--bg-raised)', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
        <div style={{ position: 'absolute', inset: 0, background: `repeating-linear-gradient(45deg, ${tc}22 0 6px, transparent 6px 12px)` }} />
        <div style={{ position: 'absolute', bottom: 2, left: 2, fontFamily: 'var(--mono)', fontSize: 7, color: tc, opacity: 0.7 }}>IMG</div>
      </div>
...
function TodayAlert({ status, title, sub }) {
  const colors = { ok: 'var(--ok)', warn: 'var(--warn)', alert: 'var(--alert)', muted: 'var(--text-2)' };
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', background: 'var(--bg-surface)', border: '1px solid var(--line)', borderLeft: `2px solid ${colors[status]}`, borderRadius: 6 }}>
      <div style={{ flex: 1 }}>
...
// ─── Capture ──────────────────────────────────
function ScreenCapture({ back, navigate }) {
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#000' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 45%, #2a2418 0%, #0e0a06 60%, #000 100%)' }}>
        {/* fake plate */}
        <div style={{ position: 'absolute', top: '38%', left: '50%', transform: 'translate(-50%, -50%)', width: 260, height: 260, borderRadius: '50%', background: 'radial-gradient(circle at 35% 35%, #5a4530, #2a1f12)', boxShadow: '0 30px 60px rgba(0,0,0,0.6), inset 0 0 0 8px rgba(255,255,255,0.04)' }}>
        ...
        {/* tip */}
        <div style={{ position: 'absolute', top: 100, left: 16, right: 16, padding: '10px 12px', background: 'rgba(11,14,19,0.7)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {/* corner brackets */}
        {[
          { top: 180, left: 30, borderTop: '1.5px solid var(--accent)', borderLeft: '1.5px solid var(--accent)' },
          { top: 180, right: 30, borderTop: '1.5px solid var(--accent)', borderRight: '1.5px solid var(--accent)' },
          { bottom: 220, left: 30, borderBottom: '1.5px solid var(--accent)', borderLeft: '1.5px solid var(--accent)' },
          { bottom: 220, right: 30, borderBottom: '1.5px solid var(--accent)', borderRight: '1.5px solid var(--accent)' },
        ].map((pos, i) => (
          <div key={i} style={{ position: 'absolute', width: 24, height: 24, ...pos }} />
        ))}

        {/* grid overlay */}
        <div style={{ position: 'absolute', top: 200, left: 30, right: 30, bottom: 240, backgroundImage: 'linear-gradient(rgba(255,255,255,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.06) 1px, transparent 1px)', backgroundSize: '33.33% 33.33%', pointerEvents: 'none' }} />

        {/* EXPO readout */}
        <div style={{ position: 'absolute', top: 160, right: 16, padding: '6px 10px', background: 'rgba(11,14,19,0.7)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 6, display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {/* bottom controls */}
        <div style={{ position: 'absolute', bottom: 50, left: 0, right: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px' }}>
          <button style={{ width: 44, height: 44, borderRadius: 10, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', color: 'var(--text-1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="8.5" cy="11" r="2"/><path d="M3 17l5-5 4 4 3-3 6 6"/></svg>
          </button>
          <div onClick={() => navigate('loading')} style={{ width: 76, height: 76, borderRadius: '50%', background: 'transparent', border: '3px solid rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', cursor: 'pointer' }}>
            <div style={{ width: 62, height: 62, borderRadius: '50%', background: 'var(--accent)', boxShadow: '0 0 30px rgba(196,181,253,0.4)' }} />
            <div style={{ position: 'absolute', top: -22, left: '50%', transform: 'translateX(-50%)', fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Capturer</div>
          </div>
          <button style={{ width: 44, height: 44, borderRadius: 10, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', color: 'var(--text-1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2L4 13h6l-1 9 9-11h-6l1-9z"/></svg>
          </button>
        </div>

        {/* close */}
        <button onClick={() => back()} style={{ position: 'absolute', top: 56, left: 16, width: 36, height: 36, borderRadius: 10, background: 'rgba(11,14,19,0.6)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.12)', color: 'var(--text-1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
// ─── Loading pipeline ──────────────────────────
function ScreenLoading({ navigate }) {
  const { useEffect } = React;
  useEffect(() => {
    const t = setTimeout(() => navigate('result'), 4000);
    return () => clearTimeout(t);
  }, []);

  const PipelineStep = ({ n, l, sub, status }) => {
    const isDone = status === 'done';
    const isActive = status === 'active';
    const c = isDone ? 'var(--ok)' : isActive ? 'var(--accent)' : 'var(--text-4)';
    return (
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, position: 'relative' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
          <div style={{ width: 26, height: 26, borderRadius: '50%', background: isActive ? 'rgba(196,181,253,0.15)' : isDone ? 'var(--ok-soft)' : 'var(--bg-surface)', border: `1.5px solid ${isActive || isDone ? c : 'var(--line-strong)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: c, fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 500, position: 'relative', zIndex: 2 }}>
            {isDone ? '✓' : n}
            {isActive && <div style={{ position: 'absolute', inset: -6, borderRadius: '50%', border: `1.5px solid var(--accent)`, animation: 'paPulse 1.6s ease-out infinite' }} />}
          </div>
          {n < 4 && <div style={{ width: 1, flex: 1, minHeight: 36, background: isDone ? c : 'var(--line)', marginTop: 4, marginBottom: 4 }} />}
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '24px 24px 32px', background: 'var(--bg-canvas)' }}>
      ...
      <div style={{ height: 200, borderRadius: 14, background: 'radial-gradient(circle at 50% 45%, #5a4530, #1a120a)', border: '1px solid var(--line-strong)', marginBottom: 28, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '25%', left: '50%', transform: 'translateX(-50%)', width: 140, height: 140, borderRadius: '50%', background: 'radial-gradient(circle at 35% 35%, #5a4530, #2a1f12)', boxShadow: '0 12px 30px rgba(0,0,0,0.6)' }}>
        ...
        <div style={{ position: 'absolute', left: 0, right: 0, height: 2, top: 0, background: 'linear-gradient(90deg, transparent, var(--accent), transparent)', boxShadow: '0 0 12px rgba(196,181,253,0.6)', animation: 'paScan 2.4s ease-in-out infinite' }} />
      ...
      <div style={{ flex: 1 }}>
        <PipelineStep n={1} l="Vision IA · Gemini 2.5 Flash" sub="Identification aliments · 4.2 s" status="done" />
        <PipelineStep n={2} l="Matching CIQUAL" sub="Embeddings · 3 185 candidats · 1.8 s" status="done" />
        <PipelineStep n={3} l="LLM Chooser" sub="Sélection finale + portions · est. 2.5 s" status="active" />
        <PipelineStep n={4} l="Scoring + persist" sub="NOVA · Hansel · OMS · Postgres" status="pending" />
      </div>

      <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid var(--line)', display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text-3)' }}>
        <span>~8.5 s / ~12 s</span>
        <span onClick={() => navigate('today')} style={{ color: 'var(--text-4)', cursor: 'pointer' }}>Annuler</span>
      </div>
    </div>
  );
}
