// Plate Analyzer — UI primitives
// Clinical / data-forward components used across all screens

const { useState, useMemo, useEffect, useRef } = React;

// ─────────────────────────────────────────────
// Ring chart — single ring with target stroke
// ─────────────────────────────────────────────
function PARing({ value = 0, target = 100, size = 64, stroke = 5, color = '#c4b5fd', track = 'rgba(255,255,255,0.08)', children, label }) {
...
// ─────────────────────────────────────────────
// Linear progress with target tick
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Status pill — ok / warn / alert / info / muted
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Stat tile (label + big num + unit + optional)
// ─────────────────────────────────────────────
function PAStat({ label, value, unit, sub, color = 'var(--text-1)', size = 'md' }) {
  const sizes = {
    sm: { value: 16, unit: 10, label: 10 },
    md: { value: 22, unit: 11, label: 10 },
    lg: { value: 32, unit: 12, label: 11 },
    xl: { value: 44, unit: 14, label: 11 },
  };
  const s = sizes[size];
  return (
    <div>
      {label && (
        <div style={{
          fontFamily: 'var(--mono)', fontSize: s.label,
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, color }}>
        <span style={{
          fontFamily: 'var(--mono)', fontSize: s.value, fontWeight: 500,
          letterSpacing: '-0.02em', lineHeight: 1,
        }}>{value}</span>
        {unit && (
          <span style={{
            fontFamily: 'var(--mono)', fontSize: s.unit,
...
// ─────────────────────────────────────────────
// NOVA bar — 1 / 2 / 3 / 4 cells, active highlighted
// ─────────────────────────────────────────────
      ...
      {[1,2,3,4].map(n => {
        ...
        return (
          <div key={n} style={{
            ...
            height: compact ? 5 : 7,
            borderRadius: 2,
            background: active ? colors[n-1] : 'rgba(255,255,255,0.08)',
            position: 'relative',
          }}>
            {active && !compact && (
              <div style={{
...
// ─────────────────────────────────────────────
// Status bar (re-themed for our dark UI)
// ─────────────────────────────────────────────
function PAStatusBar({ time = '08:42' }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      padding: '14px 30px 8px', position: 'relative', zIndex: 20,
      color: 'var(--text-1)',
    }}>
      <span style={{
...
// ─────────────────────────────────────────────
// Phone shell — our own dark frame
// ─────────────────────────────────────────────
function PAPhone({ children, tabBar, statusBar = true, scroll = true, bg = 'var(--bg-canvas)' }) {
  return (
    <div style={{
      width: 390, height: 844, borderRadius: 54,
      background: '#000',
      padding: 10,
      boxShadow: '0 60px 120px rgba(0,0,0,0.5), 0 0 0 1.5px rgba(255,255,255,0.06), inset 0 0 0 1px rgba(255,255,255,0.04)',
      position: 'relative',
    }}>
      <div style={{
        ...
        <div className="phone-scroll" style={{
          ...
          paddingTop: statusBar ? 44 : 0,
          paddingBottom: tabBar ? 92 : 0,
...
Object.assign(window, {
  PARing, PASpark, PABar, PAPill, PAStat, PACard, PANova, PASection,
  PAStatusBar, PATabBar, PAPhone,
});
